...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const loanInstallmentValue = document.getElementById(
    'report-loan-installment-value'
  );
  const loanAmountValue = document.getElementById('report-loan-amount');
  const loanInstallmentsValue = document.getElementById(
    'report-loan-installments'
  );
  const installmentValueExample = document.getElementById(
    'report-installments-value-example'
  );
  const interestAmountValue = document.getElementById('report-interest-amount');
  const loanAmountExampleValue = document.getElementById(
    'report-loan-amount-example'
  );
  const valueMinusApplyValueExample = document.getElementById(
    'report-loan-value-minus-apply-value-example'
  );
  const inputButtons = document.querySelectorAll('.btn-check');

  simulateButton.addEventListener('click', function () {
    const totalValue =
      convertToFloor(document.getElementById('amount-loan').value) || 0;
    const downPayment =
      convertToFloor(document.getElementById('apply-value').value) || 0;
    const interestRate =
      convertToFloor(document.getElementById('interest-rate').value) || 0;
    const installments =
      parseInt(document.getElementById('installments').value) || 0;

    if (totalValue <= 0 || installments <= 0) {
      showToast(
        'Por favor, insira valores válidos para o valor total e quantidade de parcelas.'
      );
      return;
    }
    ...
    const formattedMonthlyInstallment = monthlyInstallment
      .toFixed(2)
      .replace('.', ',');
